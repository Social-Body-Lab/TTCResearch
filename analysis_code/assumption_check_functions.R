...
level_one_heteroscedasticity_random_intercept_only = function(predictor, predictor_name, outcome, level_two_variable, mydata) {
  ...
  form2 = as.formula(paste("~", 1, "|", level_two_variable))
...
#function for getting the model comparisons object used to check level-one homoscedasticity (random intercept only models)
get_comparisons_random_intercept_only = function(predictor, predictor_name, outcome, level_two_variable, mydata) {
...
get_outliers = function(comp.models, predictor, pred_mean, pred_cutoff, level_two_variable, mydata){
  ...
  outliers = subset(mydata, mydata[level_two_variable] %in% names(strongs))
  outliers[level_two_variable] = droplevels(outliers[level_two_variable])
...
test_homoscedasticity = function(predictor, outcome, level_two_variable, mydata){
  ...
  cat(paste("Homoscedasticity test p-value (significance indicates heteroscedasticity):", round(p_val, 3)), file = 'assumption_tests.txt', append=TRUE, sep = "\n")
...
level_one_linearity = function(predictor, outcome, level_one_variable, level_two_variable, mydata){
  ...
  #get within-group OLS residuals, and add it to the main dataset (droppigh all NAs)
  resi = residuals(olselist) 
  mydata$residuals = resi
  mydata = mydata[complete.cases(mydata$residuals), ]
  
  #make the plot
  jpeg('level_one_linearity.jpg', width = 5, height = 5, units = "in",res = 300, pointsize = 12, quality = 100)
  plot(mydata[, predictor], mydata$resi, xlab = expression('Level-one explanatory variable'), ylab = "Residual")
  lines(lowess(mydata[, predictor], mydata$resi))
  dev.off()
...
ols_normality = function(predictor, outcome, level_one_covariate, level_two_variable, mydata){
  ...
  #disregard the (binary) level-one covariate
  level_one_covariate = NULL
...
level_two_outliers = function(model_comparison, predictor, outcome, level_two_variable, mydata){
  ...
  #location of predictor variable
  predictor_binary = 2
  ...
  if (length(outliers) == 1){
    ...
    outlier_subset = nrow(subset(mydata, mydata[[level_two_variable]] %in% outlier_names))
    ...
    #this will give the percentage of these participants who have a total number of observations larger than the average
    counter = 0
    for (i in outlier_names) {
      if (mean_observations < nrow(subset(mydata, mydata[[level_two_variable]] == i)))
        counter = counter + 1
    }
    ...
    #this will give the percentage of participants who have a total number of observations larger than the average
    counter = 0
    for (i in outlier_names) {
      if (mean_observations < nrow(subset(mydata, mydata[[level_two_variable]] == i)))
        counter = counter + 1
    ...
    cat(paste("Total level-two outliers:",  length(outliers)), file = 'assumption_tests.txt', append=TRUE, sep = "\n")
    cat(paste("Mean total observations per level-two unit:", mean_observations), file = 'assumption_tests.txt', append=TRUE, sep = "\n")
    cat(paste("Mean observations for outlying level-two units:", round(mean_outlier_observations, 2)), file = 'assumption_tests.txt', append=TRUE, sep = "\n")
    cat(paste("Percentage of outliers with fewer observations than the overall average:", round(percentage_with_few_obs, 2)), file = 'assumption_tests.txt', append=TRUE)
...
check_influence = function(lme_model, level_two_variable){
  ...
  cat(paste("Mean Cook's distances:", round(cooks_distance_mean, 3)), file = 'assumption_tests.txt', append=TRUE, sep = "\n")
  cat(paste("Standard deviation of Cook's distances:", round(cooks_distance_sd, 3)), file = 'assumption_tests.txt', append=TRUE, sep = "\n")
...
check_influence_with_exclusion = function(lme_model, predictor, level_two_variable, mydata){
  ...
  #get whether the main predictor variable changed significance (this assumes that the main predictor variable is the first in the model formula)
  changed_sig = subset[, 3]
...
### CHECK ALL MAIN MULTILEVEL MODEL ASSUMPTIONS - VARIABLE CHANGE OVER TIME MODELS ###

#a function for checking all assumptions for the main multilevel models
check_assumptions_change_over_time_models = function(code_dir, mlms, mlm_dir_names, predictors, predictors_integers, predictor_names, outcomes, level_two_variable, level_two_residual_variable, level_one_variable = NULL, all_data){
  ...
  for (m in mlms){
    ...
    outcome = outcomes[[model_count]]
    mydata = all_data[[model_count]]
    
    if (cur_model == "tension_change_model"){
      level_two_normality_group = level_two_residual_variable
    } else {
      level_two_normality_group = level_two_variable
    }
      
    #level-two residual normality
    print("LEVEL-TWO NORMALITY")
    if (length(m@cnms[level_two_variable]) == 1) {
      level_two_residual_normality(m, level_two_normality_group)
    } else {
      level_two_residual_normality(m, level_two_normality_group, predictor)
    }
    
    #level-two outliers
    print("LEVEL-TWO OUTLIERS")
    comparisons = get_comparisons_random_intercept_only(predictor, predictor_name, outcome, level_two_variable, mydata)
    level_two_outliers(comparisons, predictor, outcome, level_two_variable, mydata)
    
    #level-two unit influence
    print("LEVEL-TWO INFLUENCE")
    ...
    print("LEVEL-ONE HOMOSCEDASTICITY - OBSERVE")
    comparisons = level_one_heteroscedasticity_random_intercept_only(predictor, predictor_name, outcome, level_two_variable, mydata)
    
    #test level-one homoscedasticity
    print("LEVEL-ONE HOMOSCEDASTICITY - TEST")
    test_homoscedasticity(predictor_int, outcome, level_two_variable, mydata)
    
    #level-one linearity
    print("LEVEL-ONE LINEARITY - TEST SKIPPED (NOT APPLICABLE)")
  #  level_one_linearity(predictor, outcome, level_one_variable, level_two_variable, mydata)
    
    #level-one residual normality
    print("LEVEL-ONE RESIDUAL NORMALITY - TEST")
...
### CHECK ALL MAIN MULTILEVEL MODEL ASSUMPTIONS - MAIN MODELS ###

#a function for checking all assumptions for the main multilevel models
check_assumptions_main_models = function(code_dir, mlms, mlm_dir_names, predictors, predictors_integers, predictor_names, outcomes, level_two_variable, level_one_variable = NULL, all_data){
